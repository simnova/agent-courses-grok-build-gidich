...
const allSteps = [
  { name: 'biome', cmd: 'pnpm exec biome check "apps/api" "packages/axc" "packages/domain" "packages/axc-verification" "packages/cellix/mongoose-seedwork"' },
  { name: 'format:check', cmd: 'pnpm exec biome format "apps/api" "packages/axc" "packages/domain" "packages/axc-verification" "packages/cellix/mongoose-seedwork" "apps/docs/src" "apps/docs/docusaurus.config.ts" "apps/docs/sidebars.ts"' },
  { name: 'analyze', cmd: 'pnpm run analyze' },
  { name: 'build', cmd: 'pnpm exec turbo run build --filter="@apps/api" --filter="@axc/*" --filter="@axc-verification/*"' },
  { name: 'test:arch', cmd: 'pnpm exec turbo run test:arch --filter="@apps/api" --filter="@axc/*" --filter="@axc-verification/*"' },
  { name: 'test:acceptance', cmd: 'pnpm exec turbo run test:acceptance --filter="@axc-verification/*"' },
  { name: 'knip', cmd: 'pnpm run knip' },
  { name: 'audit', cmd: 'pnpm run audit' },
  { name: 'snyk', cmd: 'pnpm run snyk || echo "snyk step completed (see policy and output; findings tolerated for initial scaffold)"' },
];
