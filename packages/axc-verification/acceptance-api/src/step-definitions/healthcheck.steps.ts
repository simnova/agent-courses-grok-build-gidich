...
When('I request the healthcheck endpoint', async () => {
...
Then('the response should indicate service is "ok"', async () => {
	await actorCalled('Tess').attemptsTo(Ensure.that(lastBody.status, equals('ok')));
});

Then('the response should include a recent timestamp', async () => {
...
Then('the response should identify service {string}', async (serviceName: string) => {
